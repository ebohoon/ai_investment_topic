...
export function buildDesignSystemPrompt(
  ...
  return [
    ...
    "- explorationExample은 주차·일정 나열(1주차에 ~, 2주차에 ~) 금지. 탐구 방법(methodSteps)을 바탕으로, 이 주제라면 어떤 구체적 대상·현상·사회문제를 다루고 어떤 자료·측정·분석(표·그래프·비교·문항 등)으로 무엇을 밝혀 볼 수 있는지 4~8문장으로 서술합니다. 내용 안내 예시) 사회문제·데이터 분석형이면 어떤 사회 이슈인지, 어떤 공개·조사 데이터의 어떤 변수를 어떻게 다루어 경향·차이를 어떻게 읽을지까지 구체적으로. 실험·관찰·인터뷰형이면 어떤 재료·대상·도구로 무엇을 관찰·기록할지 구체적으로. 미완료를 완료한 것처럼 단정하지 않습니다.",
    "- explorationExample 문자열은 반드시 맨 앞이 '예:'로 시작합니다(앞에 공백 없음, '예:' 다음 한 칸 띄운 뒤 본문).",
...
export const FEW_SHOT_DESIGN_ASSISTANT = JSON.stringify(
  {
    ...
    explorationExample:
      "예: 탐구 질문이 공공시설 '접근성' 인식과 배치의 차이라면, 학교가 속한 구의 어린이집·도서관·운동장 등 시설 좌표를 공공데이터포털 또는 지자체 열린데이터에서 확보해 지도에 올리고, '집에서 도보 15분 이내' 같은 기준으로 구역별로 몇 개씩 있는지 개수를 센다. 별도로 동년배 5명에게 '가장 가고 싶은 공공시설'과 '멀다고 느끼는 이유'를 짧은 설문(익명)으로 받아 빈도를 표로 묶은 뒤, 시설 밀도가 높은 구역과 응답을 대응해서 보며 '완전한 원인'으로 단정하지 않고 표본·지역 단위의 한계를 한 문장으로 남긴다.",
    expectedResults:
